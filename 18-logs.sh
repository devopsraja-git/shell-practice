...
if [ $? -ne 0 ]; then
    ...
    echo -e "mySQL is already exists....$Y SKIPPING... $N" &>> $LOG_FILE
fi
...
if [ $? -ne 0 ]; then
    ...
    echo -e "Nginx is already exists...$Y SKIPPING... $N" &>> $LOG_FILE
fi
...
if [ $? -ne 0 ]; then
    ...
    echo -e "Python is already existing... $Y SKIPPING...$N" &>> $LOG_FILE
fi